...
class DiseaseParser(
    ...
    fun import(
        ...
        while (stream.peek() is Token.Heading3) {
            ...
            val contraction =
                stream.consumeUntil {
                    it is Token.BoldPart && (it.text.startsWith("Incubation:") || it.text.startsWith(
                        "Duration:"
                    ))
                }

            val incubation = if (stream.peek()?.text?.startsWith("Incubation:") == true) {
                stream.consumeOne()
            ...
            stream.consumeOne()
            val symptoms =
                stream.consumeOneOfType<Token.NormalPart>().text.split(",").map { it.trim() }
                    .filter { it.isNotBlank() }
            val permanentEffects = if (stream.peek() is Token.BoldPart) {
                stream.consumeOne()
                stream.consumeOneOfType<Token.NormalPart>().text.trim()
            } else {
                ""
