package cz.frantisekmasa.wfrp_master.common.core.firestore
...
fun <T : Any> documentFlow(
    ...
    val listener = document.addSnapshotListener { snapshot, exception ->
        exception?.let { trySend(snapshotProcessor(Either.Left(it))) }
        snapshot?.let {
            launch {

                val data = withContext(Dispatchers.Default) {
                    snapshotProcessor(if (snapshot.exists()) Either.Right(it) else Either.Left(null))
                }
